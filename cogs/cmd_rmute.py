...
class class_rmute(commands.Cog):
    ...
    """@commands.slash_command(
        name="rmute", description="Specify the role of the muted | Указать роль мьюта"
    )
    @commands.guild_only()
    ...
    async def rmute(
        ...
        if enabled:
            ...
            if option is not None:
                if option and role:
                    ...
                    if option == 1:
                        if str(ctx.guild.id) not in w:
                            ...
                            embed = discord.Embed(
                                description=f"{get_language(ctx.guild.id,'<a:vega_check_mark:821700784927801394> Для мьюта указана роль:')} {role.mention}",
                                color=0x4ACB84,
                            )
                            ...
                            if c is not None:
                                embed = discord.Embed(
                                    description=f"{get_language(ctx.guild.id,'<a:vega_x:810843492266803230> Роль мьюта уже указана')} — {c.mention}!",
                                    color=0xCC1A1D,
                                )
                                ...
                                embed = discord.Embed(
                                    description=f"{get_language(ctx.guild.id,':warning: Прежняя роль мьюта была удалена. Пожалуйста, укажите её снова.')}",
                                    color=0xFCC21B,
                                )
                                embed.add_field(
                        ...
                        if str(ctx.guild.id) in w:
                            del w[str(ctx.guild.id)]
                            embed = discord.Embed(
                                description=f"{get_language(ctx.guild.id,'<a:vega_x:810843492266803230> Роль мьюта убрана!')}",
                                color=0xCC1A1D,
                            )
                            await ctx.send(embed=embed, delete_after=15.0)
                        else:
                            embed = discord.Embed(
                                title=f"{get_language(ctx.guild.id,':warning: Роль мьюта не указана!')}",
                                color=0xFCC21B,
                            )
                            await ctx.send(embed=embed, ephemeral=True)
                    else:
                ...
                embed = discord.Embed(
                    description=f"{get_language(ctx.guild.id,'<a:loupe:811137886141153320> Укажите опцию и роль!')}",
                    color=0x8899A6,
                )
                embed.add_field(
                    name=f"{get_language(ctx.guild.id,'Описание:')}",
                    value=f"{get_language(ctx.guild.id,'Администраторы сервера могут указать 1 роль мьюта для бота.')}",
                    inline=False,
                )
                embed.add_field(
