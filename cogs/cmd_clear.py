...
class class_clear(commands.Cog):
    ...
    @commands.slash_command(
        name="clear", description="Clear messages | Очитстить сообщения"
    )
    @commands.guild_only()
    ...
    async def clear(
        ...
        limit: int = commands.Param(
            description="Number of messages to delete | Количество собщений для удаления"
        ),
    ):
        ctx = inter
        try:
        ...
        if enabled:
            ...
            if limit < 1:
                embed = discord.Embed(
                    description=f"{get_language(ctx.guild.id,'❗️ Минимальное количество сообщений для очистки **1**!')}",
                    color=0xBE1931,
                )
                ...
                embed = discord.Embed(
                    description=f"<a:b_loading:857131960223662104> {get_language(ctx.guild.id,'Пожалуйста подождите, выполняется очистка')}<a:loading:929703010781757472>",
                    color=0xF4900C,
                )
                await ctx.send(embed=embed, ephemeral=True)
                # try:
                ...
                if 200 < limit < 1e10:
                    embed = discord.Embed(
                        description=f"{get_language(ctx.guild.id,'❗️ Максимальное количество сообщений для очистки **200**!')}",
                        color=0xBE1931,
                    )
                    await ctx.send(embed=embed, ephemeral=True)
                """if user is None:
