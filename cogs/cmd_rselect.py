...
class class_rselect(commands.Cog):
    ...
    @commands.slash_command(name="rselect", description="Take the role from all users | Забрать роль у всех пользователей", case_insensitive=True)
    @commands.guild_only()
    @commands.bot_has_permissions(send_messages=True, manage_roles=True)
    @commands.cooldown(1, 300, commands.BucketType.guild)
    async def rselect(self, ctx, option: option = commands.Param(description="Select an option | Укажите опцию"), role: discord.Role=commands.Param(name="role", description="Specify the role or its ID | Укажите роль или её ID")):
        try:
            ...
            if await checkchannel(ctx):
                if ctx.author == ctx.guild.owner:
                    if option == 1 and role:
                        ...
                        embed = discord.Embed(description=f"{get_language(ctx.guild.id,'<a:b_loading:857131960223662104> Пожалуйста подождите, выполняется изъятие роли у пользователей...')}", color=0xf4900c)
                        ...
                        try:
                            ...
                            if member:
                                embed = discord.Embed(description=f"{get_language(ctx.guild.id,'<a:vega_check_mark:821700784927801394> Я забрал у всех роль')} {role.mention}", color=0x43b581)
                                try:await ctx.edit_original_message(embed=embed)
                                except:pass
                        except:
                            embed = discord.Embed(description=f"{get_language(ctx.guild.id,'<a:vega_x:810843492266803230> Я не смог изъять у всех роль')} {role.mention}.\n— {get_language(ctx.guild.id,'Видимо моя роль находится ниже роли')} {role.mention}.", color=0xcc1a1d)
                            try:await ctx.edit_original_message(embed=embed)
                            except:pass
