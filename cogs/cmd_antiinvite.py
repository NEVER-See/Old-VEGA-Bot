...
class class_antiinvite(commands.Cog):
    ...
    @commands.slash_command(
        name="antiinvite",
        description="Anti-invitation function | Функция Анти приглашения",
    )
    @commands.guild_only()
            ...
            if ctx.author == ctx.guild.owner:
                ...
                if option is not None:
                        ...
                        if option != None:
                            ...
                            if option == 1:
                                if not enabled:
                                    ...
                                    embed = discord.Embed(
                                        description=f"{get_language(ctx.guild.id,'<a:vega_check_mark:821700784927801394> Анти приглашения были включены на вашем сервере!')}",
                                        color=0x43B581,
                                    )
                                    await ctx.send(embed=embed, delete_after=12.0)
                                else:
                                    embed = discord.Embed(
                                        description=f"{get_language(ctx.guild.id,':warning: Анти приглашения уже включены на вашем сервере!')}",
                                        color=0xFCC21B,
                                    )
                                ...
                                if enabled:
                                    ...
                                    embed = discord.Embed(
                                        description=f"{get_language(ctx.guild.id,'<a:vega_x:810843492266803230> Анти приглашения были отключены на вашем сервере!')}",
                                        color=0xCC1A1D,
                                    )
                                    await ctx.send(embed=embed, delete_after=12.0)
                                else:
                                    embed = discord.Embed(
                                        description=f"{get_language(ctx.guild.id,':warning: Анти приглашения уже отключены на вашем сервере!')}",
                                        color=0xFCC21B,
                                    )
                                    await ctx.send(embed=embed, delete_after=10.0)
                                    ctx.command.reset_cooldown(ctx)
                    ...
                    embed.add_field(
                        ...
                        value=f"{get_language(ctx.guild.id,'Вы можете включить или отключить анти приглашения. Действует по всем каналам!')}",
                        inline=False,
                    )
                    embed.add_field(
                    ...
                    embed.add_field(
                        ...
                        value=f"{ctx.prefix}antiinvite on",
                        inline=False,
                    )
                    embed.set_footer(
