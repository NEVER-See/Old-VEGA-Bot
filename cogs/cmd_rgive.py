...
class class_rgive(commands.Cog):
    ...
    @commands.slash_command(
        name="rgive",
        description="Assign a role to all users | Выдать роль всем пользователям",
        ...
        if enabled:
            ...
            if ctx.author == ctx.guild.owner:
                if option == 1 and role:
                    ...
                    embed = discord.Embed(
                        description=f"{get_language(ctx.guild.id,'<a:b_loading:857131960223662104> Пожалуйста подождите, выполняется раздача роли пользователям...')}",
                        color=0xF4900C,
                    )
                    ...
                    try:
                        ...
                        if member:
                            embed = discord.Embed(
                                description=f"{get_language(ctx.guild.id,'<a:vega_check_mark:821700784927801394> Я выдал всем роль')} {role.mention}",
                                color=0x43B581,
                            )
                            try:
                        ...
                        embed = discord.Embed(
                            description=f"{get_language(ctx.guild.id,'<a:vega_x:810843492266803230> Я не смог выдать всем роль')} {role.mention}.\n— {get_language(ctx.guild.id,'Видимо моя роль находится ниже роли')} {role.mention}.",
                            color=0xCC1A1D,
                        )
                        try:
                            await ctx.edit_original_message(embed=embed)
