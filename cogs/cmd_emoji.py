...
class class_emoji(commands.Cog):
    ...
    async def emoji(
        self,
        inter,
        see: str = commands.Param(
            name="emoji",
            description="Specify the server emoji | Укажите серверный эмодзи",
        ),
    ):
        ctx = inter
        emoji = see
            ...
            if emoji is not None:
                if emoji:
                    ...
                    if em is not None:
                        ...
                        embed = discord.Embed(
                            title=f"{get_language(ctx.guild.id,'Эмодзи:')} {em}",
                            color=0x2F3136,
                        )
                    ...
                    #            await ctx.message.delete()
                        ...
                        embed = discord.Embed(
                            description=f"{get_language(ctx.guild.id,':warning: Неизвестное эмодзи!')}",
                            color=0xFCC21B,
                        )
                ...
                embed = discord.Embed(
                    description=f"{get_language(ctx.guild.id,'<a:loupe:811137886141153320> Укажите эмодзи!')}",
                    color=0x8899A6,
                )
                embed.add_field(
                    name=f"{get_language(ctx.guild.id,'Описание:')}",
                    value=f"{get_language(ctx.guild.id,'Можно указывать только те эмодзи, которые загружены на сервер.')}",
                    inline=False,
                )
                embed.add_field(
                ...
                embed.add_field(
                    ...
                    value=f"{ctx.prefix}emoji <:python:826158844555427891>",
                    inline=False,
                )
                embed.set_footer(
